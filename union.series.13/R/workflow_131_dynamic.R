...
setMethod("step2", signature = c(x = "job_dynamic"),
  function(x, dir_results, fun_detect = .detect_gromacs_xvg_project.hb, ...)
  {
    ...
    p.energy <- set_lab_legend(
      ...
      glue::glue(
        "体系总能量变化曲线|||展示各复合物体系在分子动力学模拟过程中总能量随时间的变化趋势。横坐标为模拟时间（ns），纵坐标为能量（根据设置可能为原始能量或组内中心化后的ΔEnergy）。图中每个子图对应一个复合物体系，并在左下角标注其平均能量及标准差。该图用于评估体系的热力学稳定性及能量收敛情况，能量波动较小通常表示体系趋于稳定，而较大波动可能提示尚未充分平衡或存在构象变化。"
      )
    )
