...
{
  base <- file.path(
    path, c("union.publish", "union.project")
  )
...
build_completions <- function(pkgs = c(basename(list_unions()), "union.utils"))
{
  invisible(lapply(pkgs, build_completion_for_package))
}
