...
setMethod_traceable("asjob_scfea", signature = c(x = "job_seurat"),
...
setMethod_traceable("step0", signature = c(x = "job_scfea"),
  function(x)
  {
    step_message("Prepare your data with function `asjob_scfea`.")
  })

setMethod_traceable("step1", signature = c(x = "job_scfea"),
  function(x)
  {
...
setMethod_traceable("step2", signature = c(x = "job_scfea"),
  function(x){
...
setMethod_traceable("vis", signature = c(x = "job_scfea"),
  function(x, group.by, feature, groups = NULL, order = TRUE,
    cutoff = "tail", mag = 100, return_type = c("job", "plot"), 
    name = NULL, try_format_group.by = TRUE)
  {
    if (missing(group.by)) {
    ...
    if (any(grpl(head(x$metadata[[group.by]]), "[ ]")) && try_format_group.by) {
      message(
        glue::glue('grpl(x$metadata[[group.by]], "[ ]"), use `formal_name` ...')
      )
      x$metadata[[group.by]] <- formal_name(x$metadata[[group.by]])
    }
...
setMethod_traceable("mutate", signature = c(x = "job_scfea"),
  function(x, ...){
    x$metadata <- dplyr::mutate(x$metadata, ...)
    return(x)
  })

setMethod_traceable("map", signature = c(x = "job_scfea", ref = "job_limma"),
    ...
    which = NULL, ...)
  {
    ...
    names(p.fluxs) <- vapply(
      which, function(x) bind(.get_versus_cell(x), co = "_"), character(1)
    )
    x$p.fluxs <- p.fluxs
    x$.feature_gene <- lapply(which,
      function(which) {
        top <- ref@tables$step2$tops[[which]]
        nl(top$name, top$gene)
      })
    names(x$.feature_gene) <- names(p.fluxs)
    return(x)
  })

setMethod_traceable("map", signature = c(x = "job_seurat", ref = "job_scfea"),
...
setMethod_traceable("cal_corp", signature = c(x = "job_limma", y = "job_seurat"),
...
setMethod_traceable("regroup", signature = c(x = "job_scfea", "job_seurat"),
  function(x, ref){
...
setMethod_traceable("asjob_limma", signature = c(x = "job_scfea"),
  function(x, metadata, group, scale_sample = TRUE, scale_var = FALSE, ...)
  {
    message(crayon::red(glue::glue("`Group` only support format generated by `as_type_group`")))
    if (missing(metadata) && !is.null(x$from_seurat)) {
      ...
      if (missing(group)) {
        ...
        metadata[[group]] <- formal_name(metadata[[group]])
      }
    }
...
setMethod_traceable("set_remote", signature = c(x = "job_scfea"),
...
setMethod_traceable("intersect", signature = c(x = "job_limma", y = "character"),
