...
)
{
  ...
  snap(data) <- glue::glue("从 eQTLGen 联盟公开的顺式表达数量性状位点（cis‑eQTL）数据中提取基因表达的工具变量。以目标基因为输入，借助以 R 包 org.Hs.eg.db ⟦pkgInfo('org.Hs.eg.db')⟧ 和 AnnotationDbi ⟦pkgInfo('AnnotationDbi')⟧ 映射至 Ensembl ID，随后通过以 R 包 TwoSampleMR ⟦pkgInfo('TwoSampleMR')⟧ 查询 OpenGWAS (<https://opengwas.io/>) 数据库，保留与表达显著关联的遗传变异。⟦mark$blue('显著性阈值设为 p < {pval_threshold}，并对工具变量执行 LD clumping（r² = {clump_r2}，窗口 = {clump_kb} kb），以保证独立性')⟧。未能匹配或缺乏显著 eQTL 的基因将被剔除，最终获得暴露因素的工具变量集合{stats}。")
  return(data)
}
...
.get_outcome_opengwas <- function(
  ...
  check_new_snps = FALSE, check_new_ids = FALSE, ask_query = FALSE
)
{
  snps <- unique(snps)
  target_ids <- unique(target_ids)
  
  # 1. Search for historic cache
  cache_files <- list.files("tmp", pattern = paste0("^", cache_prefix), full.names = TRUE)
  
  cached_data <- NULL
  ever_seen_ids <- character(0L)
  ever_seen_snps <- character(0L)
  
  # Check if we have any prior history
  has_history <- length(cache_files) > 0L

  if (has_history) {
    cached_list <- lapply(cache_files, readRDS)
    cached_data <- data.table::rbindlist(cached_list, use.names = TRUE, fill = TRUE)
    ...
    message(glue::glue("SNPs: {less(snps, n = 5)}"))
    message(glue::glue("Datasets ID: {less(target_ids, n = 5)}"))
    if (!sureThat("No cache file esits, to download data now?")) {
      stop('!sureThat, stop fetch network data.')
    }
  }

  # 2. Logic Control: If no history exists, we MUST fetch everything
  # Otherwise, respect the user's toggle switches
  ...
  # Define Gaps based on the forced or toggled logic
  new_snps <- character(0L)
  if (fetch_new_snps_now) {
    new_snps <- setdiff(snps, ever_seen_snps)
  }
  
  new_ids <- character(0L)
  if (fetch_new_ids_now) {
    new_ids <- setdiff(target_ids, ever_seen_ids)
  }
  ...
  # 3. Inner Fetching Engine
  .fetch_and_cache <- function(s_list, id_list, tag) {
    if (length(s_list) == 0L || length(id_list) == 0L) return(NULL)
    
    message(glue::glue("Task [{tag}]: Fetching {length(s_list)} SNPs across {length(id_list)} IDs"))
    
    id_chunks <- split(id_list, ceiling(seq_along(id_list) / 30L))
    
    tmp_res <- pbapply::pblapply(id_chunks, function(chunk) {
      expect_local_data(
        "tmp", cache_prefix, 
        function(s, o) {
          d <- TwoSampleMR::extract_outcome_data(snps = s, outcomes = o)
          # Polite pause for the API
          Sys.sleep(3L)
          return(d)
        },
        list(s = s_list, o = chunk)
      )
    })
    return(tmp_res)
  }

  # 4. Execute Incremental or Full Fetch
  ...
  final_list <- final_list[sapply(final_list, function(x) !is.null(x) && nrow(x) > 0L)]
  ...
  merged_all <- data.table::rbindlist(final_list, use.names = TRUE, fill = TRUE)
  
  # Final precision filter for the current request
  res <- dplyr::filter(merged_all, id.outcome %in% target_ids, SNP %in% snps)
...
)
{
  ...
  all_templates <- list(
    smoking = "Smoking initiation|Lifetime smoking|Cigarettes per day",
    drinking = "Drinks per week|Alcohol intake frequency|Alcohol consumption",
    BMI = "^Body mass index$|BMI",
    education = "Years of schooling|Educational attainment",
    crp = "^C-reactive protein$|CRP",
    t2d = "Type 2 diabetes",
    ibd = "Inflammatory bowel disease|Crohn|Ulcerative colitis",
    coffee = "Coffee intake",
    sleep = "Sleep duration|Insomnia",
  ...
  snap(res) <- glue::glue("从 OpenGWAS 目录中筛选具有代表性的 GWAS summary 数据。具体实现为：以正则表达式对性状描述、数据库信息进行匹配，识别与目标表型相关的所有可用数据集；经去重后，以样本量（sample_size）和工具变量数目（SNP）为优先准则降序排列，保留排序靠前的前 {top_n} 个条目，以确保纳入分析的数据兼具统计效力与遗传变异数目优势。")
  return(res)
}
...
)
{
  ...
  snap(res) <- glue::glue("将暴露因素工具变量 (SNP) 中与{type}因素显著关联（P < {signif(p_threshold, 2)}）的 SNP 视为受{type}影响并予以排除。对于所有暴露因素的 SNP，最终剔除 {length(bad_snps)} 个{type}因素 SNP，保留 {nrow(res)} 个 SNP。")
  return(res)
}
...
)
{
  ...
  glue::glue(
    "在完成预设质控与敏感性筛选后，共保留 {n_pair} 组具有统计学支持的暴露因素—结局关联结果。其中⟦mark$red('正向效应 {n_pos} 组，负向效应 {n_neg} 组')⟧，提示候选暴露因素对结局可能同时存在促进性与保护性影响模式。
