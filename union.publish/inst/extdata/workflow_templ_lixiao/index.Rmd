...
```{r eval = T, echo = F, results = "asis"}
code("
  git clone https://github.com/shaman-yellow/union.publish.git ~/union/union.publish
  ")
```

`union.publish` 是标准的 R 包结构形式，这意味着，即使你不用 `git` 获取它，
单纯用：

- `remotes::install_github("shaman-yellow/union.publish")`

也能成功获取并 直接安装完成。但是，这包里面大多数的方法都没有导出 (export) 到用户层次
(这是因为，这个包的改动情形太多了，我一直在创建新的方法或者调整旧的方法) ，
你即使 `library(union.publish)` 加载了它，也会出现使用不了许多方法的情况。
万无一失的做法是：
