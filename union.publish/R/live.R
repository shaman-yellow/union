...
.feature_join <- setClass("feature_join", contains = c("feature_list"))
.feature_alias <- setClass("feature_alias", contains = c("feature_list"))

setAs("feature_list", "feature_join",
  function(from, to) {
    setNames(.feature_join(from), names(from))
  }
)
  ...
  function(x){
    ...
    dplyr::mutate(tibble::as_tibble(data.frame(x)),
      ...
      significant = ifelse(pvalue > .05, "> 0.05",
        ifelse(pvalue > .01, "< 0.05",
          ifelse(pvalue > 0.001, "< 0.01", "< 0.001"))),
